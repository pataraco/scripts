...
-- 2.1.0   Review the EXAMPLE Database

-- 2.1.1   Navigate to [Databases], then locate and select EXAMPLE.
...
-- 2.1.7   Using the breadcrumb trail above the table list, click EXAMPLE to
...
-- 2.1.11  Use the breadcrumb trail to go back to EXAMPLE.
...
-- 2.2.2   Locate and click LEARNING in the database list.
...
-- 2.2.11  Create a schema in your database, and name it LEARNING.
...
CREATE SCHEMA LEARNING.LEARNING;
...
-- 2.4.4   Execute the following query which selects MIN and MAX values for the
...
-- 2.4.5   Notice that the results return almost immediately.

-- 2.4.6   Click Query ID in the result frame, then click the Query ID.
--         Results Query Id

-- 2.4.7   This will bring up the query detail pages under Query History.

-- 2.4.8   From within the history, select the [Profile] tab above the summary
...
-- 2.5.2   Using SQL, create a warehouse named LEARNING2, with the following
--         characteristics.
...
--         Answer: All students are working in the LEARNING role, and all
...
-- 2.5.15  Locate LEARNING2 and verify the Status is now Started.
...
-- 2.5.17  Suspend LEARNING2:

ALTER WAREHOUSE LEARNING2 SUSPEND;
...
-- 2.5.19  Alter LEARNING2 and set the WAREHOUSE_SIZE to XSmall:

ALTER WAREHOUSE LEARNING2 SET WAREHOUSE_SIZE=XSmall;


-- 2.5.20  Suspend and then drop LEARNING2:
...
--         LEARNING2 cannot be suspended.
--         Proceed with the statement below to drop the warehouse:

DROP WAREHOUSE LEARNING2;


-- 2.5.21  Show warehouses to verify that LEARNING is still listed, but
--         LEARNING2 is not.
