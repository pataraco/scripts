...
GRANT USAGE ON WAREHOUSE LEARNING TO ROLE SYSADMIN;
USE ROLE SYSADMIN;
...
-- 5.3.14  Set your context to LEARNING role and warehouse, 
--         and SNOWFLAKE_SAMPLE_DATA, TPCH_SF100 database and schema:
...
-- 5.3.17  Change the line AS sum_qty to AS sum_qty_new and run
--         the query again. What do you think will happen?

SELECT l_returnflag, l_linestatus,
    SUM(l_quantity) AS sum_qty_new,
    SUM(l_extendedprice) AS sum_base_price,
    SUM(l_extendedprice * (l_discount)) AS sum_disc_price,
    SUM(l_extendedprice * (l_discount) * (1+l_tax)) AS sum_charge,
    AVG(l_quantity) AS avg_qty,
