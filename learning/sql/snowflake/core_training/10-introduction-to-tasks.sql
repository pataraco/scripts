...
AS    
    INSERT INTO LEARNING.TASKS.SHIPPING_BY_DATE (ORDERDATE, SHIPDATE, DAYSTOSHIP)
    SELECT 
        O_ORDERDATE
        , L_SHIPDATE
        , L_SHIPDATE - O_ORDERDATE AS FULFILL_DATE
    FROM
        EXAMPLE.TPCH_SF1.ORDERS O LEFT JOIN EXAMPLE.TPCH_SF1.LINEITEM L ON O.O_ORDERKEY = L.L_ORDERKEY     
    WHERE 
        YEAR(O_ORDERDATE) = (SELECT yr FROM LEARNING.TASKS.year_month WHERE ID = (select MAX(ID) from year_month))
        AND 
        MONTH(O_ORDERDATE)= (SELECT mon FROM LEARNING.TASKS.year_month WHERE ID = (select MAX(ID) from year_month));
...
--         the SELECT statement pulls only data with an O_ORDERDATE that matches
--         the month and year most recently inserted into the year_month table.
--         Thus, each minute it will pull one month’s worth of data.
...
AS  
    INSERT INTO LEARNING.TASKS.AVG_SHIPPING_IN_DAYS(yr, mon, avg_shipping_days)
    SELECT
        YEAR(orderdate) AS yr
      , MONTH(orderdate) AS mon
      , AVG(daystoship) AS avg_shipping_days
    FROM
        LEARNING.TASKS.SHIPPING_BY_DATE
    WHERE
        yr = (SELECT yr FROM LEARNING.TASKS.year_month WHERE ID = (select MAX(ID) from year_month))
        AND
        mon = (SELECT mon FROM LEARNING.TASKS.year_month WHERE ID = (select MAX(ID) from year_month))
    GROUP BY 
        YEAR(orderdate)
      , MONTH(orderdate);
...
$$
    var r='continue';
    var mon = snowflake.createStatement({sqlText: 'SELECT mon FROM LEARNING.TASKS.year_month WHERE ID = (select MAX(ID) from year_month);'});
