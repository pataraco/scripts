...
--         you. The stage is in the TRAININGLAB schema of the EXAMPLE
...
DESCRIBE STAGE EXAMPLE.TRAININGLAB.ED_STAGE;
...
LIST @EXAMPLE.traininglab.ed_stage/load/lab_files/ pattern='.*region.*';
...
COPY INTO REGION
FROM @EXAMPLE.traininglab.ed_stage/load/lab_files/
FILES = ('region.tbl')
...
-- 9.3.1   Empty the REGION Table in the PUBLIC schema of LEARNING:
...
-- 9.4.4   Your file should have saved to the location you selected. Examine the
--         file using the text editor of your choice. The file should have a
--         header row and five rows of data.

-- 9.4.5   Now let’s create a pipe format using the user interface. In the Web
...
-- 9.6.8   Suspend and resize the warehouse
