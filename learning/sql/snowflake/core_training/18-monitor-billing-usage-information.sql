...
--         ACCOUNTADMIN permission. The role LEARNING has been granted
...
-- 18.2.1  Set your role to LEARNING (if not already set).
...
-- 18.2.7  Locate and click on your LEARNING and evaluate your usage over
--         time.
...
--         granted to LEARNING for convenience in this exercise.
...
--         LEARNING Virtual Warehouse, ordered by time:

SELECT *
FROM TABLE(INFORMATION_SCHEMA.warehouse_metering_history
(date_range_start=>date_trunc(month, current_date),
date_range_end=>dateadd(month,1,date_trunc(month, current_date)),'LEARNING'))
ORDER BY start_time;
...
--         each Table in the WEATHER Schema in the EXAMPLE Database. Sort by
--         largest table:
