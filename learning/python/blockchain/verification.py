...
class Verification:
    def generate_guess_string(self, transactions, last_block_hash, proof):
    ...
    def valid_proof(self, transactions, last_block_hash, proof):
        guess_str = self.generate_guess_string(transactions, last_block_hash, proof)
    ...
    def valid_tx(self, transaction, get_balance):
        ...
        for i, block in enumerate(blockchain):
            if i == 0:
                ...
                if self.valid_proof(
                    ...
                    is_valid = False
                print()
                guess_str = self.generate_guess_string(
                    txs_without_mining_reward, block.prev_block_hash, block.proof,
                )
    ...
    def validate_txs(self, transactions, get_balance):
        """ Verifies all transactions given. """
        return all([self.valid_tx(t, get_balance) for t in transactions])
