...
started_at=`date +%T`
start_hr=`expr ${started_at:0:2} + 0`
start_mn=`expr ${started_at:3:2} + 0`
start_sc=`expr ${started_at:6:2} + 0`
days=0
hr_delta=0
ping_fails=0
ping_passes=0
...
if [ $? -eq 0 -o $USE_IP = "TRUE" ]; then
   ...
   while true; do
      ...
      if [ $PING = "TRUE" -a $NC = "TRUE" ]; then
         ...
         state=ALLPASS
      elif [ $PING = "FALSE" -a $NC = "FALSE" ]; then
         THC=$RED
      ...
      cur_time=`date +%T`
      cur_hr=`expr ${cur_time:0:2} + 0`
      cur_mn=`expr ${cur_time:3:2} + 0`
      cur_sc=`expr ${cur_time:6:2} + 0`
      ...
      if [ $hr_delta -ne 0 -a $cur_hr -le $start_hr ]; then             # adjust
         hr_delta=$(( 24 - start_hr + cur_hr + (days * 24) ))
      else
         hr_delta=$(( cur_hr - start_hr + (days * 24) ))
      fi
      [ $hr_delta -ne 0 -a $((hr_delta%24)) -eq 0 ] && (( days += 1 ))
