...
#   Must have 'aws' and 'jq' installed
#
#   Example ~/.aws/config
#
#      [profile dev]
...
if [ -n "$PROFILE" ]; then
   ...
   if [ "$PROFILE" == "unset" ]; then
      ...
      if [ $? -eq 0 ]; then
         export AWS_ACCESS_KEY_ID=$(jq -r .Credentials.AccessKeyId $AWS_STS_CREDS)
         export AWS_SECRET_ACCESS_KEY=$(jq -r .Credentials.SecretAccessKey $AWS_STS_CREDS)
         export AWS_SESSION_TOKEN=$(jq -r .Credentials.SessionToken $AWS_STS_CREDS)
